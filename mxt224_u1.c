...
// transform the raw data into string to ouput it
int data2str(int *pData, int nDataSize, char *pStrBuf, int nStrBufSize)
{
	int n = 0;
	int i = 0;
	...
	// set to zero
	memset(pStrBuf, 0, sizeof(pStrBuf[0])*nStrBufSize );

	// format to string
	for (i=0; i<nDataSize; ++i)
	{
		n += sprintf(&pStrBuf[n], "%d,", pData[i]);

		if(n >= nStrBufSize) // no enough string buff
		{
			printk(KERN_ERR "[yl] no enough string buff.\n");
			return -1;
		}
	}
...
// the reference data is Excess-k-format, while delta is in a format of two's complment
int compute_raw_data(uint16_t *pRef, uint16_t *pDelta, int *pRawData, int nNodeNum)
{
	int nRef = 0;
	int nDelta = 0;
...
// get object info from object table
...
// set the DIAGNOSTIC field in T6 object
